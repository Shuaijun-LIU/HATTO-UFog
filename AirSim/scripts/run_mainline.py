...
def _plot_user_point(client, airsim, *, x_ned: float, y_ned: float, z_ned_ground: float, label: str) -> None:
    ...
    # Use a tall marker so it remains visible across different maps where "ground z" may not be exactly 0.
    z0 = float(z_ned_ground)
    p0 = airsim.Vector3r(float(x_ned), float(y_ned), z0 + 20.0)   # slightly below typical ground
    p1 = airsim.Vector3r(float(x_ned), float(y_ned), z0 - 220.0)  # tall column above (NED negative is up)
    try:
        client.simPlotLineList([p0, p1], color_rgba=[1.0, 0.2, 0.2, 1.0], thickness=12.0, duration=-1, is_persistent=True)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="AirSim mainline (ExternalPhysicsEngine) video run.")
    ...
    parser.add_argument("--z_up_m", type=float, default=5.0)
    parser.add_argument("--x0", type=float, default=None, help="Optional: start X (NED meters). Defaults to current spawn X.")
    parser.add_argument("--y0", type=float, default=None, help="Optional: start Y (NED meters). Defaults to current spawn Y.")
    parser.add_argument("--target_x", type=float, default=None, help="Optional: target X (NED meters).")
    parser.add_argument("--target_y", type=float, default=None, help="Optional: target Y (NED meters).")
    ...
    parser.add_argument(
        "--ignore_collision",
        action="store_true",
        help="Pass ignore_collision=True to simSetVehiclePose (useful for safe teleport; demo-quality path should still avoid clipping).",
    )
    ...
    try:
        ...
        print("")
        print("ERROR: Failed to connect to AirSim RPC server for mainline.")
        print("- Mainline requires AirSim to be restarted with ExternalPhysicsEngine enabled.")
    ...
    # Mainline: we directly set pose each step (ExternalPhysicsEngine recommended).
    # AirSim uses NED: z_down positive. Our inputs are z-up meters -> z_ned = -z_up.
    ignore_collision = bool(args.ignore_collision)
    pose = client.simGetVehiclePose(vehicle_name=args.vehicle)
    spawn_x: float = float(pose.position.x_val)
    spawn_y: float = float(pose.position.y_val)

    # Auto-pick start/target for showcase so UAV/goal are not always on the road or identical across demos.
    # User can still override x0/y0/target_x/target_y explicitly.
    # For stability, we sample a few candidate flightplans and pick the one with least collisions
    # by teleport-sampling collision penetration along the candidate path.
    if args.x0 is not None and args.y0 is not None and (args.target_x is not None and args.target_y is not None):
        # Explicit override path anchor and target (kept for debugging).
        fp = choose_flightplan(
            run_id=ctx.run_id,
            spawn_xy_ned=(float(args.x0), float(args.y0)),
            prefer_offroad=False,
            scene_profile=str(args.scene_profile),
        )
        fp = type(fp)(
            start_xy_ned=(float(args.x0), float(args.y0)),
            target_xy_ned=(float(args.target_x), float(args.target_y)),
            z_up_m=float(fp.z_up_m),
        ...
        z_up_m = float(args.z_up_m) if args.z_up_m is not None else float(fp.z_up_m)
        z_ned = -float(z_up_m)
        ...
        def scorer(cand):
            z_up = float(args.z_up_m) if args.z_up_m is not None else float(cand.z_up_m)
            z_ned_local = -float(z_up)
            ...
            return score_pose_path(client=client, airsim=airsim, vehicle_name=args.vehicle, pose_points=pose_pts, sample_every=12)
    ...
    pose.position.x_val = float(x0)
    pose.position.y_val = float(y0)
    ...
    for step in range(args.steps):
        x, y, yaw = path_xy_yaw[step]
        pose.position.x_val = x
        pose.position.y_val = y
        pose.position.z_val = z_ned
        pose.orientation = airsim.to_quaternion(0.0, 0.0, float(yaw))
        client.simSetVehiclePose(pose, ignore_collision, vehicle_name=args.vehicle)
        ...
        if args.overlay:
            ...
            vy = 0.0
            if step > 0:
                x_prev, y_prev, _ = path_xy_yaw[step - 1]
                vx = (float(x) - float(x_prev)) / float(args.dt)
                vy = (float(y) - float(y_prev)) / float(args.dt)
            ...
            overlay = [
                OverlayLine("MAINLINE (pose-driven)", (255, 255, 255), scale=1.05),
                OverlayLine(f"step={step}/{args.steps - 1}   t={float(step) * float(args.dt):.2f}s", (220, 220, 220)),
                OverlayLine(f"pos_ned=({float(x):.1f},{float(y):.1f},{float(z_ned):.1f})   yaw={float(yaw):+.2f} rad", (255, 255, 0)),
        ...
        rows.append(
            {
                ...
                "x_ned": float(pose.position.x_val),
                "y_ned": float(pose.position.y_val),
