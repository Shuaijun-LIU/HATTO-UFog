...
from framework.baselines.base import Action
from framework.alloc import make_resource_allocator
from framework.baselines import make_baseline
from framework.config import Config
from framework.env.generators import generate_world
from framework.env.metrics import compute_metrics
from framework.env.tasks import generate_md_positions, sample_tasks
from framework.env.queue import QueueState
from framework.io import export_world
from framework.logging.parquet_logger import ParquetLogger
from framework.logging.run_writer import make_run_id, prepare_run_dir, write_config, write_summary
from framework.seeding import make_rng
from framework.sim.dynamics import KinematicState, move_towards, movement_energy
from framework.sim.energy import movement_energy_from_omegas
from framework.sim.rigid_body import RigidBodyState, RigidBodyModel, euler_from_quat
from framework.sim.control_loop import LowLevelController
from framework.utils import shortest_path
...
class Simulator:
    def __init__(self, cfg: Config, world=None) -> None:
        ...
        if self.cfg.dynamics.mode == "rigid_body":
            ...
            self.low_level = LowLevelController(self.cfg.control, self.cfg.dynamics)
    ...
    def _safe_target(self, target: Tuple[float, float, float]) -> Tuple[Tuple[float, float, float], bool]:
        ...
        if self.world.segment_is_free(cur, target, step=self.world.cfg.connect_step_m):
            return target, False
        # Try waypoint graph routing if available
        if self.world.waypoints and self.world.waypoints.nodes:
            graph = self.world.waypoints
            start = self._nearest_node(cur)
            goal = self._nearest_node(target)
            path = shortest_path(graph.nodes, graph.edges, start, goal)
            if path and len(path) > 1:
                return graph.nodes[path[1]], True
        # Fallback: lift altitude above terrain
        x, y, z = target
        safe_z = self.world.terrain.height(x, y) + self.world.terrain.clearance_m + self.cfg.sim.reroute_altitude_margin_m
    ...
    def _step_kinematic(self, target: Tuple[float, float, float]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        viol_obstacle = 0
        viol_alt = 0
        if not self.world.is_free(pos_tuple):
            viol_obstacle = 1
            x, y, z = pos_tuple
            safe_z = self.world.terrain.height(x, y) + self.world.terrain.clearance_m + self.cfg.sim.recovery_altitude_margin_m
            new_state.pos[2] = max(new_state.pos[2], safe_z)
            if not self.world.is_free(tuple(new_state.pos)):
                new_state = self.state
        if new_state.pos[2] > self.world.cfg.height_m or new_state.pos[2] <= self.world.terrain.height(new_state.pos[0], new_state.pos[1]) + self.world.terrain.clearance_m:
            viol_alt = 1
    ...
    def _step_rigidbody(self, target: Tuple[float, float, float]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        for _ in range(steps):
            ...
            self.rb_state, _thrust, _tau = self.rb_model.step(self.rb_state, ctrl.omegas, dt)
            ...
            if not self.world.is_free(pos_tuple):
                viol_obstacle = 1
                # lift if below terrain
                x, y, z = pos_tuple
                safe_z = self.world.terrain.height(x, y) + self.world.terrain.clearance_m + self.cfg.sim.recovery_altitude_margin_m
                self.rb_state.pos[2] = max(self.rb_state.pos[2], safe_z)
            if self.rb_state.pos[2] > self.world.cfg.height_m or self.rb_state.pos[2] <= self.world.terrain.height(self.rb_state.pos[0], self.rb_state.pos[1]) + self.world.terrain.clearance_m:
                viol_alt = 1
            last_ctrl = ctrl
    ...
    def step(self, external_action: Optional[Action] = None) -> Tuple[Dict[str, Any], bool]:
        ...
        if self.cfg.dynamics.mode == "rigid_body":
            _, meta = self._step_rigidbody(target)
        else:
            _, meta = self._step_kinematic(target)
