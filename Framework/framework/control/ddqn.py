...
def train_ddqn_gain_adjuster(output_dir: str, cfg: DDQNConfig | None = None) -> dict:
    ...
    for ep in range(cfg.episodes):
        ...
        for t in range(cfg.steps_per_episode):
            ...
            error_meas = error + noise
            derror = error_meas - prev_error_meas
            prev_error_meas = error_meas
            E = float(np.mean(error_meas))
            EC = float(np.mean(derror))
            if use_delta:
            ...
            next_error_meas = next_error + next_noise
            next_derror = next_error_meas - error_meas

            reward = -(
                cfg.reward_w_e * float(np.mean(np.abs(next_error)))
                + cfg.reward_w_de * float(np.mean(np.abs(next_derror)))
                + cfg.reward_w_du * float(np.mean((u - prev_u) ** 2))
            )
            ...
            next_E = float(np.mean(next_error_meas))
            next_EC = float(np.mean(next_derror))
            if use_delta:
